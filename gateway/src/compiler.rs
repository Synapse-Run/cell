...
impl Parser {
    ...
    fn parse_expr(&mut self) -> Node {
        ...
        if tok == "write8" {
            ...
            let val = self.parse_expr();
            return Node::Call {
                callee: "__store8".into(),
                args: vec![addr, val],
            };
        }
        if tok == "memory_size" {
            return Node::Call {
                callee: "__memory_size".into(),
                args: vec![],
            };
        }
        if tok == "memory_grow" {
        ...
        if tok == "store" {
            ...
            let val = self.parse_expr();
            return Node::Call {
                callee: "__f32_store".into(),
                args: vec![ptr, idx, val],
            };
        }
        if tok == "load" {
            let ptr = self.parse_expr();
...
fn parse_program(source: &str) -> Result<Program, String> {
    ...
    let ordered_imports: Vec<(String, FfiImport)> = import_order
        .into_iter()
        .map(|k| {
...
fn build_standard_ffi() -> HashMap<String, FfiImport> {
    ...
    m.insert(
        "print".into(),
        ffi("env", "print", vec![0x7F, 0x7F], vec![]),
    );
    m.insert(
        "print_i64".into(),
        ffi("env", "print_i64", vec![0x7E], vec![]),
    );
    m.insert(
        "print_f32".into(),
        ffi("env", "print_f32", vec![0x7D], vec![]),
    );
    m.insert(
        "tcp_connect".into(),
        ffi("env", "tcp_connect", vec![0x7E, 0x7E], vec![0x7E]),
    );
    m.insert(
        "tcp_send_raw".into(),
        ffi("env", "tcp_send_raw", vec![0x7E; 3], vec![0x7E]),
    );
    m.insert(
        "p2p_receive_compute".into(),
        ffi("env", "p2p_receive_compute", vec![0x7E; 2], vec![0x7E]),
    );
    m.insert(
        "tcp_recv_raw".into(),
        ffi("env", "tcp_recv_raw", vec![0x7E; 3], vec![0x7E]),
    );
    m.insert(
        "tcp_bind".into(),
        ffi("env", "tcp_bind", vec![0x7E], vec![0x7E]),
    );
    m.insert(
        "crypto_sign".into(),
        ffi("env", "crypto_sign", vec![0x7F; 4], vec![]),
    );
    m.insert(
        "crypto_verify".into(),
        ffi("env", "crypto_verify", vec![0x7F; 4], vec![0x7F]),
    );
    m.insert(
        "tcp_accept".into(),
        ffi("env", "tcp_accept", vec![0x7E], vec![0x7E]),
    );
    m.insert(
        "tcp_close".into(),
        ffi("env", "tcp_close", vec![0x7E], vec![0x7E]),
    );
    m.insert(
        "p2p_receive".into(),
        ffi("env", "p2p_receive", vec![0x7E; 3], vec![0x7E]),
    );
    m.insert(
        "p2p_respond_compute".into(),
        ffi("env", "p2p_respond_compute", vec![0x7E; 3], vec![0x7E]),
    );
    m.insert(
        "p2p_request_compute".into(),
        ffi("env", "p2p_request_compute", vec![0x7E; 3], vec![0x7E]),
    );
    m.insert(
        "gpu_matmul".into(),
        ffi("env", "gpu_matmul", vec![0x7E; 6], vec![0x7E]),
    );
    m.insert(
        "gpu_relu".into(),
        ffi("env", "gpu_relu", vec![0x7E; 2], vec![0x7E]),
    );
    m.insert(
        "gpu_softmax".into(),
        ffi("env", "gpu_softmax", vec![0x7E; 3], vec![0x7E]),
    );
    m.insert(
        "gpu_silu".into(),
        ffi("env", "gpu_silu", vec![0x7E; 2], vec![0x7E]),
    );
    m.insert(
        "gpu_layernorm".into(),
        ffi("env", "gpu_layernorm", vec![0x7E; 4], vec![0x7E]),
    );
    m.insert(
        "gpu_add".into(),
        ffi("env", "gpu_add", vec![0x7E; 3], vec![0x7E]),
    );
    m.insert(
        "load_weights".into(),
        ffi("env", "load_weights", vec![0x7E; 4], vec![0x7E]),
    );
    m.insert(
        "weight_info".into(),
        ffi("env", "weight_info", vec![0x7E], vec![0x7E]),
    );
    m
}
...
impl Emitter {
    ...
    fn has_f32_literal(node: &Node) -> bool {
        match node {
            ...
            Node::BinaryExpr { left, right, .. } => {
                Self::has_f32_literal(left) || Self::has_f32_literal(right)
            }
            Node::IfStmt {
                test,
                consequent,
                ...
                Self::has_f32_literal(test)
                    || Self::has_f32_literal(consequent)
                    || alternate.as_ref().is_some_and(|a| Self::has_f32_literal(a))
            }
            Node::WhileStmt { test, body } => {
                Self::has_f32_literal(test) || Self::has_f32_literal(body)
            }
    ...
    fn compile_expr(&mut self, node: &Node) -> Vec<u8> {
        match node {
            ...
            Node::VarDecl { name, init } => {
                ...
                if val_type == "f32" && declared == "i64" {
                    result.push(0xAE);
                } else if val_type == "i64" && declared == "f32" {
                    result.push(0xB4);
                }
    ...
    fn compile_call(&mut self, callee: &str, args: &[Node]) -> Vec<u8> {
        ...
        if let Some(&idx) = self.func_index.get(callee) {
            ...
            let ffi_param_types: Option<Vec<u8>> = self
                .ffi_imports
                .iter()
                .find(|(n, _)| n == callee)
            ...
            if let Some(param_types) = ffi_param_types {
                for (i, arg) in args.iter().enumerate() {
                    ...
                    if i < param_types.len() {
                        ...
                        // i32.wrap_i64
                        else if expected == 0x7D && arg_type != "f32" {
                            result.push(0xB4);
                        } else if expected == 0x7E && arg_type == "f32" {
                            result.push(0xAE);
                        }
                    }
                }
...
pub fn compile_syn(source: &str) -> Result<Vec<u8>, String> {
    ...
    let mut get_or_add_type = |params: Vec<u8>, results: Vec<u8>| -> usize {
        ...
        if let Some(&idx) = type_map.get(&key) {
            return idx;
        }
    ...
    // Import type indices
    let import_type_indices: Vec<usize> = program
        .ffi_imports
        .iter()
    ...
    let user_type_indices: Vec<usize> = func_names
        .iter()
        .map(|(name, arity)| {
            let is_f32 = func_is_f32.get(name).copied().unwrap_or(false);
    ...
    for (name, func) in &program.functions {
        ...
        let locals_decl = if num_extra > 0 {
            ...
            let idx_to_name: HashMap<usize, String> = emitter
                .local_env
                .iter()
