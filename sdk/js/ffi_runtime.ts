...
export function createFfiImports(state: EdgeExecutionState) {
    return {
        env: {
            ...
            gpu_layernorm: () => -1n,
            gpu_add: () => -1n,
            load_weights: (dst_ptr: bigint, weight_id: bigint, offset: bigint, size: bigint): bigint => {
                const dst = Number(dst_ptr);
                const off = Number(offset);
                const sz = Number(size);
                
                const MAX_MODEL_SIZE = 89456640;
                if (off < 0 || sz <= 0 || off + sz > MAX_MODEL_SIZE) return -2n; // OOB

                if (!state.memory) return -3n; // No memory
