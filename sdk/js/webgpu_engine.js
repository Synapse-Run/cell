...
export async function dispatchCompute(shaderName, dims, // usually 4 uint32s
inputArrays, outputSize, // in elements
workgroups = [1, 1, 1]) {
...
export async function gpu_ternary_matmul(x, w, m, n, k) {
    const TILE = 16;
...
export async function gpu_silu(x, length) {
...
export function np_rms_norm(x, w, eps = 1e-6) {
    let mean_sq = 0;
...
export function apply_rope(q, k, pos, num_heads, num_kv_heads, head_dim, theta = 1000000.0) {
...
export class QwenLayerGPU {
    ...
    constructor(tensors, i, apply_sparsity = false) {
...
export class KVCache {
    ...
    pos;
    head_dim;
    constructor(max_seq_len, kv_dim, head_dim) {
...
export function loadQwenFromSafetensors(tensors, config) {
    const layers = [];
    const kv_caches = [];
    for (let i = 0; i < config.num_layers; i++) {
        ...
        layers.push(new QwenLayerGPU(tensors, i, config.apply_sparsity));
...
export async function forward_one(model, token, layer_start = 0, layer_end = model.config.num_layers - 1, incoming_state) {
    ...
    for (let li = layer_start; li <= layer_end; li++) {
        ...
        for (let i = 0; i < mlp_in.length; i++)
            mlp_in[i] = gate_silu[i] * up[i];
