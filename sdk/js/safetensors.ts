...
export type TensorMap = Record<string, Float32Array | Int8Array | Int32Array>;

export async function fetchSafetensors(url: string, progressCallback?: (percent: number) => void): Promise<TensorMap> {
...
export function parseSafetensors(buffer: ArrayBuffer): TensorMap {
    ...
    const headerObj = JSON.parse(headerStr) as Record<string, TensorDefinition | string>;
    ...
    for (const [key, value] of Object.entries(headerObj)) {
        ...
        const def = value as TensorDefinition;
...
function _decodeFloat16(binary: number): number {
