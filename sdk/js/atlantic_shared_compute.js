 ...
 */
export function hashHiddenState(state) {
 ...
 */
export function createLayerReceipt(nodeId, privateKey, modelId, layerStart, layerEnd, inputState, outputState, computeMs) {
    const receipt = {
        version: 1,
 ...
 */
export function verifyReceiptChain(chain, publicKey, knownNodeIds) {
    const errors = [];
    ...
    for (let i = 0; i < chain.receipts.length - 1; i++) {
        if (chain.receipts[i].output_hash !== chain.receipts[i + 1].input_hash) {
            errors.push(`Hash discontinuity between shard ${i} (output: ${chain.receipts[i].output_hash.slice(0, 16)}...) ` +
    ...
    for (let i = 0; i < chain.receipts.length; i++) {
        ...
        try {
            ...
            const valid = crypto.verify(null, Buffer.from(JSON.stringify(payload)), keyObject, Buffer.from(signature, 'hex'));
            if (!valid) {
 ...
 */
export function createFleetConfig(modelId, totalLayers, nodes) {
...
if (require.main === module) {
    ...
    for (let i = 0; i < dim; i++)
        inputState[i] = Math.sin(i * 0.01) * 0.5;
    const midState1 = new Float32Array(dim);
    for (let i = 0; i < dim; i++)
        midState1[i] = inputState[i] * 1.1 + 0.3; // Simulated layer 0-7 output
    const midState2 = new Float32Array(dim);
    for (let i = 0; i < dim; i++)
        midState2[i] = midState1[i] * 0.9 - 0.1; // Simulated layer 8-15 output
    const finalState = new Float32Array(dim);
    for (let i = 0; i < dim; i++)
        finalState[i] = midState2[i] * 1.5 - 0.2; // Simulated layer 16-23 output
