...
// ─── Lease Validation ──────────────────────────────────────────────
export function validateLease(lease, publicKeyPem) {
    // 1. Check expiry
...
export function checkFeatureAccess(ffiName, lease) {
...
export function signLease(privateKeyPem, tier, nodeId) {
    const now = Math.floor(Date.now() / 1000);
    const lease = {
        version: 1,
        tier,
...
if (require.main === module) {
    ...
    // Test tampered lease
    console.log("\nTamper Test:");
    const tampered = { ...lease, tier: 'enterprise' };
